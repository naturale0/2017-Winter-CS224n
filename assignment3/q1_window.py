...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(len(sentence)):
            import itertools
            window_features = list(itertools.chain.from_iterable(padded_sentence[i: i + (2 * window_size + 1)]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(lookup_table, self.input_placeholder)
    ...
    def add_loss_op(self, pred):
        ...
        ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
