...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True] * len(sentence) + [False] * (max_length - len)
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embedding_lookup = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embedding_lookup, self.input_placeholder)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.boolean_mask(ce_loss, self.mask_placeholder))
        ### END YOUR CODE
        return loss
